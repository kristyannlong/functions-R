...
```{r}
for (value in 1:5){
  # paste is a concactenation function
...
# note that the col and row placement matters.  you get different info
```
...
```{r}
# use when we want to either test things using small data so you can prep for large data or do the same thing over and over.  

our_function <- function(optional_params){
  # do some stuff
}
# to call the function
our_function(# pass in your data)
...
# create a function that converts from C to F
...
# for margin, 1 is columns, 2 is rows
...
1.  Do 5 CodeWars problems in R. Share your solutions here:
    ...
    array_plus_array <- function(arr1, arr2) {
      sum(arr1) + sum(arr2)
    }

    # Function to remove all numbers that repeat an odd number of times
    oddOnesOut <- function(numbers) {
      counts <- table(numbers)

      odd_counts <- names(counts)[counts %% 2 != 0]

      result <- numbers[!numbers %in% odd_counts]
    ...
    # Break a chocolate bar into 1"x1" squares
    ...
    remove_smallest <- function(numbers) {
      ...
      min_index <- which(numbers == min_value)[1] 
